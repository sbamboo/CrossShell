...
# params (raw)
  # extended syntax
  $param_extendedSyntax = $false
  if ($in -like "*-es*" -or $in -like "*-extendedsyntax*") {
...
if ($param_extendedSyntax -ne $true) {
  ...
  # Run
  $c = "while $elemA " + '{CheckAndRun-Input "' + "$(removePlaceholders($elemB))" + '"}'
  $stringBuild = rawstring("$c")
} else {
  # Run
  $stringBuild = rawstring("while $elemA $elemB")
}
iex("$stringBuild")